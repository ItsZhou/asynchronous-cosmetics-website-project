 <?php
	...
	function mod002_executeQuery( $strSQL, $arAttributes ) {
        ...
        if ( $result = $link -> query( $strSQL ) ) {    // Esta funcion en vez de emplear progrmacion procedimental(vease v01 mod_002) se emplea programacion orientado a objetos
	...
	function mod002_getUsuarios() {
        ...
        $strSQL = "SELECT idusuario, nomusuario, apellidos
                    FROM USUARIOS";
    ...
    function mod002_getFotoUsuarios( $idUser ) {
		$arAttributes =[
            [ "idusuario",		"idUser",		"int"       ],
			[ "foto",		    "photoUser",                ],
    ...
    function mod002_getDetalleUsuarios( $idUser ) {
		$arAttributes =[
            [ "idusuario",		"idUser",		"int"       ],
        ...
        $strSQL = "SELECT idusuario, direccion, email, telefono
                    FROM USUARIOS
                    WHERE idusuario = $idUser";
...
//dump( $arRetorno );
    ...
    function mod002_saveNewUser( $nameUser, $surnameUser, $birthUser, $addressUser, $emailUser, $passwordUser ) {
        $link = mod001_conectoBD ();
        ...
        if ( $result = $link -> query( $strSQL ) ) {  // Aqui en el 'if' se ha empleado programacion orientado a objetos
    ...
    function mod002_getMarcasSearch( $itemToSearch ) {
        $arAttributes =[
            [ "idmarca",            "idBrand",           "int"   ],
            [ "nommarca",           "nameBrand"                  ],
        ];

        $strSQL = "SELECT idmarca, nommarca
                    FROM  MARCAS
                    WHERE nommarca LIKE '%$itemToSearch%'";
    ...
    function mod002_getProductosSearch( $itemToSearch ) {
        $arAttributes =[
            [ "idproducto",            "idProduct",           "int"   ],
            [ "nomproducto",           "nameProduct"                  ],
        ];

        $strSQL = "SELECT idproducto, nomproducto
                    FROM  PRODUCTOS
                    WHERE nomproducto LIKE '%$itemToSearch%'";
